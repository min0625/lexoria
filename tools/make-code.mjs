...
if (cmd === "keygen") {
	...
	console.log(
		`私鑰已寫入 ${pemPath.pathname}（不進版控，遺失＝該 kid 再也簽不了新碼）`,
	);
...
if (effect.type === "level") {
	const { count } = JSON.parse(
		readFileSync(new URL("../data/levels/index.json", import.meta.url)),
	);
	// 前端 validator 管不到兌換碼，關卡範圍在簽發當下把關（設計文件 §9）
	if (effect.id > count)
		die(`第 ${effect.id} 關不存在，目前只到第 ${count} 關`);
}
...
if (flags.exp === "none") {
	...
	const iso = /^\d{4}-\d{2}-\d{2}$/.test(flags.exp)
		? `${flags.exp}T23:59:59+08:00`
		: flags.exp;
...
function soleKid() {
	const pems = existsSync(KEYS_DIR)
		? readdirSync(KEYS_DIR).filter((f) => f.endsWith(".pem"))
		: [];
...
const pemPath = new URL(`${kid}.pem`, KEYS_DIR);
if (!existsSync(pemPath))
	die(`找不到私鑰 ${pemPath.pathname}——先跑 make-code.mjs keygen ${kid}`);
...
console.log(`\n${token}\n`);
console.log(
	"把整串 token 給玩家，貼進設定卡的兌換欄位即可（沒有兌換連結，見設計文件 §9）。",
);
