...
const enable = readFileSync(
	new URL("./data/enable1.txt", import.meta.url),
	"utf8",
)
	.split("\n")
	.map((w) => w.trim())
	.filter((w) => VALID_WORD.test(w));
const wordinfo = JSON.parse(
	readFileSync(new URL("./data/wordinfo.json", import.meta.url), "utf8"),
);
...
function tryLayout(words, rng) {
	...
	const rs = placements.flatMap((p) =>
		cellsOf(p.word, p.row, p.col, p.dir).map((c) => c.r),
	);
	const cs = placements.flatMap((p) =>
		cellsOf(p.word, p.row, p.col, p.dir).map((c) => c.c),
	);
...
function buildLevel(id, base, band, rng) {
	...
	const nTargets =
		band.targets[0] +
		Math.floor(rng() * (band.targets[1] - band.targets[0] + 1));
...
function validate(level) {
	...
	for (const b of level.bonus)
		if (targets.includes(b)) fail(`${b} 同時是目標與 bonus`);
	...
	for (const word of [...targets, ...level.bonus]) {
		...
		for (const [ch, n] of count([...word])) {
			if ((pool.get(ch) ?? 0) < n)
				fail(`${word} 無法由字母 ${level.letters} 組成`);
		}
	}
...
const bandCands = BANDS.map((band, bi) =>
	shuffle(
		Object.keys(wordinfo).filter(
			(w) => w.length === band.len && wordinfo[w].z >= band.baseZipf,
		),
		mulberry32(1000 + bi),
	),
);
...
for (let id = 1; id <= N_LEVELS; id++) {
	...
	let level = null;
	while (!level) {
		if (bandCursor[bi] >= bandCands[bi].length)
			throw new Error(`level ${id}: 基底字候選用罄`);
...
for (const l of levels)
	writeFileSync(
		new URL(`${l.id}.json`, levelsDir),
		`${JSON.stringify(l, null, 2)}\n`,
	);
writeFileSync(
	new URL("index.json", levelsDir),
	`${JSON.stringify({ count: levels.length })}\n`,
);
