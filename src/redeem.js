...
export async function verifyCode(
	...
	const jwk =
		t && Object.hasOwn(keys, t.header.kid) ? keys[t.header.kid] : null;
	...
	if (typeof jti !== "string" || !isValidEffect(effect))
		return { ok: false, reason: "invalid" };
	// 專屬碼：payload 帶 uid 就只有那台裝置能兌（沒帶＝活動碼，行為不變）。
	// 玩家改 localStorage 的 uid 即可繞過——同既有取捨。
	if (boundUid !== undefined && boundUid !== uid)
		return { ok: false, reason: "wrongUid" };
