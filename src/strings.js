...
// noHint 排在額外單字之後：兩句都在時「我破了第 5 關，額外找到 3 個單字，全程沒用提示」
// 是先報成果再補條件，順序反過來會把最不重要的插在中間、讀起來像三個並列的短句。
const shareHead = (n, bonus, noHint) =>
	bonus === null
		? `我正在玩 Lexoria 第 ${n} 關`
...
export const strings = {
	...
	notAWord: (w) => `${w} 不是單字`,
	levelClear: "過關！",
	// 過關卡片上的徽章：整關沒用過提示才出現。寫「零提示」不寫「完美」——後者要先定義什麼算完美
	// （bonus 字沒找完算不算？），而這句是玩家自己驗得了的事實。
	noHintClear: "零提示過關",
	// 每 50 關一次，只在第一次破關時出現（重玩不給，理由見 main.js onWin）。
	// 不寫「第 50 關」——標題那行已經寫了，徽章要補的是「走到這裡本身是件事」。
	milestone: (n) => `${n} 關達成`,
	nextLevel: "下一關",
	bonusFound: (n) => `額外單字 +${n}`,
	// 本關 0 個 bonus 時頂替上面那句：慶祝卡不該印「+0」，而且前 100 關有 15 關根本沒有
	// bonus 可找。用「累計」不用「總共」——前者讀得出還會繼續加上去。
	bonusTotal: (n) => `累計額外單字 ${n}`,
	// 過關卡片上金幣圖示的 aria-label：圖示是 CSS mask、本身沒有文字，
	// 少了這個字，讀螢幕的人聽到的就只有「+10」（見 main.js onWin）。
	coinUnit: "金幣",
	noCoins: "金幣不足",
	tutorial: "滑過字母，連成單字",
	rotateDevice: "請轉直畫面",
	...
	about: "關於",
	// 「重複」不能省：重玩時**沒找過**的 bonus 字照樣給 +2（獎勵按關卡記錄，一個字一輩子只給一次，
	// 所以沒有刷金幣迴圈）。少了那兩個字就是假話——玩家會一邊看到這句、一邊看到金幣變多。
	// 被扣掉的只有過關金幣（onWin 的 !replay），主文件 §7 的原話也是「不重複給金幣」。
	...
	shareText: (n, letters, grid, bonus, noHint) =>
		`${shareHead(n, bonus, noHint)}\n\n只有 ${letters.join(" ")} ${"三四五六七"[letters.length - 3] ?? letters.length}個字母\n能填滿這些 ⬜ 嗎？\n\n${grid}\n`,
	...
	// 主詞用「分享內容」不用「文案」——後者是行銷／工程的說法，玩家不會這樣稱呼自己剛複製的東西。
	shareCopied: "分享內容已複製，貼上就能傳給朋友",
