...
function playOnce(el, cls) {
	...
	const off = (e) => {
		...
		if (
			...
			el.getAnimations().some((a) => a.animationName === cls)
		)
			return;
		el.classList.remove(cls);
		ac.abort();
...
document.addEventListener(
	...
	(e) => {
		...
		if (
			...
			!e.target.closest("button, input, label, a, summary")
		)
			e.preventDefault();
...
document.addEventListener(
	"touchmove",
	(e) => {
		if (!e.target.closest(".level-list, .card, .dict-card, input"))
			e.preventDefault();
	},
	{ passive: false },
);
...
async function startLevel(id) {
	...
	wheel = createWheel($("wheel"), level.letters, {
...
function bonusLine(levelBonusCount) {
	...
	const total = Object.values(save.foundBonusWords).reduce(
		(n, ws) => n + ws.length,
		0,
	);
...
function onWin() {
	...
	const badges = [];
	if (!replay && currentLevelId % 50 === 0)
		badges.push(strings.milestone(currentLevelId));
...
function updateClaim() {
	...
	if (ready) {
		...
		btn.textContent = strings.claimSeconds(
			Math.min(59, Math.ceil(remainingMs / 1000)),
		);
		...
		btn.setAttribute(
			"aria-label",
			strings.claimWait(Math.floor(m / 60), m % 60),
		); // 「3:59」讀不出倒數意涵
		// 平常每分鐘醒一次就好；快進最後一分鐘時提早醒來銜接秒級倒數
		claimTimer = setTimeout(
			updateClaim,
			Math.min(60_000, remainingMs - CLAIM_FINAL_MS),
		);
	}
	claimWasReady = ready;
}
...
wireShare($("btn-share-allclear"), strings.shareScore, () =>
	strings.shareAll(),
);
...
(async function boot() {
	...
	if (save.currentLevel > levelCount)
		$("gate-level").textContent = strings.allClear;
