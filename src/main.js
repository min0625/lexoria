...
//
// 這個檔案**必須 loop，而且不能是數位全零**——兩者都是實機（iOS 靜音模式 + 無痕分頁）換來的：
//   - 只播一次不夠：250ms 播完 session 就掉回去。
//   - 全零的無聲也不夠：改成 loop 之後 `AudioContext` 仍然一路卡在 suspended，所有 `start()`
//     都靜靜地沒聲音，直到玩家答對一個字、TTS 真的唸出人聲才把 session 喚醒。兩次量到的
//     `resume()` 是 **8974ms** 與 **5124ms**，而且都恰好在 TTS 的 `onend` 之後才完成——推論是
//     系統只認「真的有波形輸出」，數位全零不算音訊活動。
// 所以檔案內容是 440Hz、峰值 2/32768（約 -84dBFS）的正弦波：任何裝置上都聽不見，但不是零。
// 這正是 #19 當年那個 10~15 秒 `resume()` 的真正成因，它從來不是幻覺，只是要在靜音模式下才現形
// （一般模式重測是 190ms，所以先前一度誤判它已不成立）。
// 順帶一提：同一招（loop）在 `<audio>` 時代是明確的退步，因為當時它要跟 12 顆音效元素的解鎖
// 搶播放管線（解鎖窗口 639ms → 1705ms）；現在頁面上只剩這一個 `<audio>`，沒有東西可搶。
