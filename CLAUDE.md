...
- [grid.js](src/grid.js), [dictionary-card.js](src/dictionary-card.js) — DOM rendering only. grid.js also exports `snapshotText` (emoji grid for the text-only share — board *shape* only, two colors, no progress; share targets tend to drop text when files are attached, so sharing sends no image) and `snapshotBlob` (canvas PNG for the manual download button — the button is currently `hidden` in index.html, code kept for later: board as spoiler-free colored blocks plus the letter wheel; colors read from CSS variables so it follows the theme). dictionary-card.js also owns `speechSynthesis` pronunciation (`speak`/`stopSpeech`), used by both its speaker button and main.js's auto-pronounce on target hit (§6.1). Its iOS unlock rule is **not** the same as the `<audio>` one below — only a `click` wakes the TTS engine (`touchend`/`pointerdown` were both measured failing) and a drag never produces one, re-locking on every page load. That is why index.html opens with the `#gate` tap-to-continue screen: it is the only guaranteed `click` in normal play, and the same tap is where the sfx `AudioContext.resume()` and the mute-switch `silence.wav` play below have to happen. Deleting the gate silently breaks both — it is not decoration. Design doc §7 has the measurements, don't re-test.
...
- Sound effects are WAV files in `assets/sfx/` (iOS Safari won't play OGG; source mapping in assets/sfx/README.md), decoded once into an `AudioContext` and played with `AudioBufferSourceNode` in main.js. The project spent a long detour on plain `<audio>` elements — `AudioContext.resume()` had measured 10-15s on device — but that unlock hung off `pointerdown`, the gesture trap §7 later documented for TTS. Re-measured once the gate guaranteed a real `click`: `resume()` 190ms, `decodeAudioData` 10ms, and `start()` **0.0ms every time**, against 6-9ms (173ms and 752ms with a cold session) for `<audio>.play()`, which blocks the main thread synchronously and was the single root of the dropped frames, swallowed sfx and undrawn selection line. Switching deleted the whole workaround stack: the per-sound element pool, the 12-element unlock loop, the silent warmup, and the gate's wait for the unlock window. Two things must stay: the `#gate` (TTS wakes only on `click`, and `resume()` must run inside a gesture) and the one-shot `silence.wav` play (the iOS mute switch silences Web Audio but not `<audio>`, WebKit 237322 — playing `<audio>` in the same gesture promotes the page's audio session so Web Audio is exempt too). Design doc §7 has the numbers and the history.
