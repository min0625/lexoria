...
- Sound effects are WAV files in `assets/sfx/` (iOS Safari won't play OGG; source mapping in assets/sfx/README.md), played via plain `<audio>` elements in main.js — not the Web Audio API, whose `AudioContext.resume()` was measured taking 10+ seconds to unlock on some iOS devices.
