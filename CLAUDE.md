...
- Sound effects are WAV files in `assets/sfx/` (iOS Safari won't play OGG; source mapping in assets/sfx/README.md), played via plain `<audio>` elements in main.js — not the Web Audio API, whose `AudioContext.resume()` was measured taking 10+ seconds to unlock on some iOS devices. Two more iOS measurements shape the code in main.js: playback unlock is counted **per element**, so every `<audio>` instance must be played once inside the first gesture; and setting `currentTime = 0` on an element that is *still playing* forces a WebKit seek that blocks the main thread 17–124ms — hence the small pool of `tick` instances (`POOL`) that lets a rapid retrigger pick an already-stopped one instead of rewinding a live one.
