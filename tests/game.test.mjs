...
const { count: levelCount } = JSON.parse(
	await readFile(new URL("index.json", levelsDir)),
);
		...
		words: [
			{
				...
				col: 0,
				dir: "across",
				def: "a small domesticated feline animal",
			},
			{
				word: "ACT",
				row: 0,
	...
	2: {
		...
		words: [
			...
			},
			{
				...
				col: 1,
				dir: "down",
				def: "to receive money in return for work",
			},
			{
				word: "RAN",
				row: 2,
...
test("useHint：金幣不足 → no-coins；足夠 → 揭示一格", () => {
	...
	assert.deepEqual(g.useHint(ECONOMY.hintCost - 1), {
		ok: false,
		reason: "no-coins",
	});
...
test("claimStatus：冷卻計時、不可累積、時鐘倒轉不鎖死", () => {
	...
	// 冷卻中：剩餘時間正確
	assert.deepEqual(claimStatus(t, t + 1 * H), {
		ready: false,
		remainingMs: 3 * H,
	});
	// 剛好滿 4 小時 → 可領
...
test("shuffleStep：走 n! 步遍歷全部並回到初始，且不退化成排名 ±1 逐格走", () => {
	for (const n of [3, 4, 7]) {
		const nFact = Array.from({ length: n }, (_, i) => i + 1).reduce(
			(a, b) => a * b,
		);
...
test("normalizeSave：合法存檔保留內容並補齊缺漏 settings", () => {
	...
	assert.equal(
		normalizeSave({ ...defaultSave(), firstOpenAt: 123 }).firstOpenAt,
		123,
	);
});
...
test("bridge.share：三態回傳與 Windows 單欄位 payload（UI 文件 §4-D）", async () => {
	...
	assert.equal(
		(await shareWith({ writes: [true], share: abort })).mode,
		"copied",
	);
	...
	assert.equal(
		(await shareWith({ writes: [false, true], share: abort })).mode,
		"copied",
	);
	// 面板本身失敗（非 AbortError）→ 不能當成送出成功，否則按鈕按了毫無反應
	assert.equal(
		(await shareWith({ writes: [false, false], share: denied })).mode,
		"failed",
	);
	assert.equal((await shareWith({ writes: [false] })).mode, "failed"); // 連面板都沒有
	// Windows 面板只讀得到單一 text 欄位 → 網址要併進去；其他平台維持分開傳（iOS 的預覽卡片）
	assert.deepEqual(
		(await shareWith({ writes: [true], share: ok, platform: "Windows" }))
			.payloads,
		[{ text: "文案\nhttps://lexoria.example/" }],
	);
	assert.deepEqual(
		(await shareWith({ writes: [true], share: ok, platform: "macOS" }))
			.payloads,
...
test("data/levels/：每關通過完整驗證", () => {
	for (const level of levels) {
		...
		assert.equal(
			new Set(level.bonus).size,
			level.bonus.length,
			`${ctx}: bonus 重複`,
		);
		for (const b of level.bonus)
			assert.ok(!targets.includes(b), `${ctx}: ${b} 同時是目標與 bonus`);
		...
		for (const word of [...targets, ...level.bonus]) {
			...
			for (const [ch, n] of count([...word])) {
				assert.ok(
					(pool.get(ch) ?? 0) >= n,
					`${ctx}: ${word} 無法由字母 ${level.letters} 組成`,
				);
			}
		}
		...
		for (const key of grid.keys()) {
			...
			for (const nk of [`${r},${c + 1}`, `${r + 1},${c}`]) {
				...
				const shared = grid
					.get(key)
					.words.some((w) => grid.get(nk).words.includes(w));
				assert.ok(
					shared,
					`${ctx}: (${key})-(${nk}) 相鄰但不同字，會產生意外字串`,
				);
			}
		}
		...
		for (const w of level.words)
			assert.ok(w.zh, `${ctx}: ${w.word} 缺中文釋義`);
	}
});
...
// 引號兩種都收：sw.js 的引號風格由 Biome 的 quoteStyle 決定，改設定不該讓這個檢查靜靜地
// 抓不到東西——比對不到就等於 SHELL 是空陣列，測試會報「少了第一支模組」而不是「regex 壞了」。
// 兩種引號各自成對（而不是 `['"]…['"]`）：後者會把不成對的引號配成一組，SHELL 裡只要有一行
// 帶 ASCII 撇號的註解就會刮出假路徑，同樣是「regex 壞了卻報成清單壞了」。
const QUOTED_RE = /"([^"]+)"|'([^']+)'/g;
const PRELOAD_RE = /modulepreload"\s+href="([^"]+)"/g;
...
const shellPaths = async () =>
	[
		...extract(SHELL_RE, await readRoot("sw.js"), "sw.js 的 SHELL")[0].matchAll(
			QUOTED_RE,
		),
	].map((m) => m[1] ?? m[2]);
...
test("sw.js SHELL 與 index.html modulepreload 涵蓋所有 src 模組，且路徑都指到真檔案", async () => {
	...
	for (const m of modules) {
		assert.ok(
			preload.includes(m),
			`index.html 的 modulepreload 少了 ${m}——模組瀑布會退回多一趟`,
		);
	}
...
test("deploy.yml 的 cp 清單涵蓋 sw.js 與 SHELL 用到的每個根目錄項目", async () => {
	...
	const staged = new Set(
		extract(CP_RE, deploy, "deploy.yml 的 cp 清單")[1].split(" "),
	);
	...
	for (const p of [...shell, "sw.js"]) {
		...
		assert.ok(
			staged.has(top),
			`deploy.yml 的 cp 清單少了 ${top}——正式站直接沒有這個檔案`,
		);
	}
});
